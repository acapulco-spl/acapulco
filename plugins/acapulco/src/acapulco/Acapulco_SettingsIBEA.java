...
public class Acapulco_SettingsIBEA extends Settings {
	...
	public static final String DEFAULT_CROSSOVER_PROBABILITY = "0.05";
	...
	public Algorithm configure(ToolInstrumenter toolInstrumenter, StoppingCondition stoppingCondition,
			Integer stoppingValue, Integer populationSize, Double mutationProbability, Double crossoverProbability, String fm,  int numFeat,
			List<List<Integer>> constr, List<ConfigurationSearchOperator> rules, IProgressMonitor monitor) throws JMException {

		populationSize_ = populationSize;
		archiveSize_ = populationSize;
		// populationSize_ = 100;
		// archiveSize_ = 100;
...
//		crossoverProbability_ = 0.05;
		...
		if (stoppingCondition == StoppingCondition.TIME) {
			algorithm = new IBEATimeLimited(problem_, stoppingValue, toolInstrumenter, monitor);
		} else {
			algorithm = new IBEAEvolutionsLimited(problem_, stoppingValue, toolInstrumenter, monitor);
		}
