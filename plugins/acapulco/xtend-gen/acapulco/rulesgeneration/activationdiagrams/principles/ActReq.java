...
public class ActReq implements ApplicationPrinciple {
  @Override
  public Set<Set<FeatureDecision>> applyTo(final FeatureDecision fd, final FeatureModelHelper fmHelper) {
    ...
    if (_isActivate) {
      ...
      {
        final Set<Feature> requiredFeatures = fmHelper.getActivableCTCFeaturesForActivateF(fd.getFeature());
        final Function1<Feature, FeatureDecision> _function = (Feature it) -> {
          return ApplicationPrinciple.activate(it);
        };
        _xblockexpression = ApplicationPrinciple.allOf(IterableExtensions.<Feature, FeatureDecision>map(requiredFeatures, _function));
      }
