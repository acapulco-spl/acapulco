...
public class DeOr implements ApplicationPrinciple {
  @Override
  public Set<Set<FeatureDecision>> applyTo(final FeatureDecision fd, @Extension final FeatureModelHelper fmHelper) {
    ...
    if (_not) {
      ...
      boolean _isORGroup = fmHelper.isORGroup(fd.getFeature().getParentFeature());
      if (_isORGroup) {
        ...
        _xifexpression_1 = ApplicationPrinciple.or(ApplicationPrinciple.oneOf(IterableExtensions.<Feature, FeatureDecision>map(IterableExtensions.<Feature>reject(fd.getFeature().getParentFeature().getOwnedFeatures(), _function), _function_1)), ApplicationPrinciple.deactivate(fd.getFeature().getParentFeature()));
      } else {
